...
function CheckThermite() {
    if(wrong === wrong_max){
        ...
        $.post(`https://apachi-vendingrobbery/thermite-callback`, JSON.stringify({ 'success': false }));
        ResetThermite();
        return;
    }
    if (right === mode_data[mode][0]) {
        StopThermiteTimer();
        $.post(`https://apachi-vendingrobbery/thermite-callback`, JSON.stringify({ 'success': true }));
        ResetThermite();
    }
}
